...
| 모듈 | 범위 | 상태 |
|---|---|---|
| `identity` | 계정 · 가족 · 구성원 · 응원 | 가입 3개·가족 7개 테스트 작성, 구현 전 |
| `fitness` | 규준 · 측정 회차 · 예측 | 미착수 |
| `activity` | 활동 기록 | 미착수 |
| `coaching` | 영상 · 코치 실행 · 미션 · 인용 | 도메인·서비스·JPA·웹 구현, 테스트 81개 (`shared.ai` Stub/Http 게이트웨이 포함) |

부모 가입부터 사용자 흐름 순서로 진행합니다. 가입 테스트 3개·가족 테스트 7개, 후속 승인 도메인 테스트 12개와 모듈 구조 테스트 3개를
작성했으며, 운영 도메인 타입은 아직 없어 테스트 컴파일이 실패하는 단계입니다.
실행 환경과 후속 순서는 [TDD 계획](../docs/tdd-plan.md)에 정리했습니다.
